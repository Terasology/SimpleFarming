...
public class TreeAuthoritySystem extends BaseComponentSystem {
     ...
     */
    @ReceiveEvent
    public void onTreePlanted(OnSeedPlanted event, EntityRef definitionEntity,
                              SaplingDefinitionComponent saplingComponent) {
     ...
     */
    @ReceiveEvent
    public void onSaplingGrowth(DelayedActionTriggeredEvent event, EntityRef sapling,
                                SaplingDefinitionComponent saplingComponent) {
     ...
     */
    @ReceiveEvent
    public void onRootGrowth(DelayedActionTriggeredEvent event, EntityRef rootEntity, LogComponent logComponent,
     ...
     */
    @ReceiveEvent
    public void onSaplingDestroyed(CreateBlockDropsEvent event, EntityRef sapling,
                                   SaplingDefinitionComponent saplingComponent) {
     ...
     */
    @ReceiveEvent
    public void onSaplingDestroyed(DoDestroyPlant event, EntityRef sapling,
                                   SaplingDefinitionComponent saplingComponent) {
    ...
    private void shiftTreeGrowth(boolean ungrowth, EntityRef rootEntity) {
        ...
        if (ungrowth) {
            if (rootComponent.growthStage == 0) {
                ...
                SaplingDefinitionComponent saplingComponent = new SaplingDefinitionComponent(rootComponent,
                        logComponent.location);
     ...
     * @param rootEntity The block entity of the root of the tree this block will be part of, used for checking
     *         if existing blocks are part of the same tree.
     ...
     * @param log The block to use for the log.
     * @param force Whether or not the log should be forced into its position, ignoring its validity.
     * @param root The root entity of the current tree, stored in the LogComponent. If it is {@link
     *         EntityRef.NULL}, this log will be set as its own root.
     * @return The newly created log's block entity.
     ...
     * @param doItemDrops Whether or not the tree should drop its items. If true, then the leaves and logs will
     *         be sent {@link DoDestroyPlant} events. Otherwise, they will just be replaced with air.
    ...
    private void destroyLog(EntityRef log, boolean doItemDrops) {
        ...
        if (doItemDrops) {
            EntityRef logItem =
                    blockItemFactory.newInstance(log.getComponent(BlockComponent.class).block.getBlockFamily(), 1);
