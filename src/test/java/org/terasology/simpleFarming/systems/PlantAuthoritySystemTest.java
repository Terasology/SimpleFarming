...
public class PlantAuthoritySystemTest extends ModuleTestingEnvironment {
   ...
   * <p>
   * Sets the target block and the block above the target to the given materials and
   * attempts to plant a seed on the target block.  Then makes assertions according
   * to the value of {@code shouldSucceed}: if {@code shouldSucceed} is true, the
   * block above the target should now be the block specified by {@code BUSH_BLOCK};
   * if not, it should remain {@code materialAbove}.  In either case, the target
